...
- FW0 provides a volatile development-load bridge after the minimum envelope prompt. Pressing `A` asks `thStorage` to reopen the single staged `.egg`, reject files outside the fixed `65536`-byte RAM capacity, read the complete file into an unavailable staging buffer, verify `PKG1` plus the header-declared file size, and close the file, FileX media, and LevelX before publishing the completed RAM image. A failed or partial load invalidates the RAM image and leaves the embedded development package as the default fallback. The package browser reports `LOADED RAM`; this is not a persistent install.
