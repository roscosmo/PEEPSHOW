...
- FW0 package-page import scaffolding remains intentionally shallow: after reclaim validates the minimum `PKG1` envelope, `thStorage` records a pending package and asks `thUI` to show a package-valid prompt. Pressing `A` requests a storage-owned install stub. Pressing `B` from valid, installed, or error package result prompts clears the prompt and returns to the menu in one press; the temporary transfer page still treats `B` as back. The stub only accepts a valid minimum envelope and still performs no erase, copy, install commit, or runtime package mutation. HW6 evidence `EV-HW6-20260813-P1-PKGMSC-043` validates the FW0 package-page force-rescan path: with `dirty=0` and `force=1`, reclaim scanned staging, classified one package file plus one host metadata directory as `PACKAGE_CANDIDATE`, validated the `PKG1` minimum envelope, ran the install stub, and returned the UI to MENU.
- HW6 evidence `EV-HW6-20260813-P1-RUNTIME-044` validates the runtime-host view of that same scaffold: package transfer enters the Platform-owned `INSTALLER / REACTIVE / RUNNING` overlay, valid package handling stays in `INSTALLER`, and install-stub completion returns to `SHELL` with no installer error. This evidence does not validate real package install commit or installed-runtime activation.
