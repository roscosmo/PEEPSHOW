...
- FW0 package-page USB transfer scaffold is another caller of that same route: `thUI` may request MSC enter/exit, but `thStorage` still owns export, host ownership, FileX/LevelX open/close, reclaim, and rescan. Package-page reclaim requests a forced staging rescan so current staging contents are inspected even when the MSC bridge did not observe a fresh write in that exact session.
...
- FW0 reclaim runs a bounded staging/export root-directory classifier after dirty MSC reclaim, and after package-page MSC reclaim when the package import scaffold explicitly forces a scan: it opens FileX/LevelX only under `thStorage`, classifies the reclaimed volume as `EMPTY`, `UNSUPPORTED`, `PACKAGE_CANDIDATE`, `MULTIPLE`, or `ERROR`, closes FileX/LevelX before returning, and performs no package import or install writes. Directories are counted for diagnostics but do not make a single package file unsupported, because host operating systems may create metadata directories. `PACKAGE_CANDIDATE` is a filename-level hint only for exactly one `.egg` file.
- FW0 then runs a read-only minimum package-envelope validator for one clean package candidate. This validator reopens the staging/export volume under `thStorage`, opens the package file for read, reads at most the first 64 bytes, requires the first four bytes to be `PKG1`, records the first 16 bytes and FileX/LevelX statuses for GDB evidence, and closes FileX/LevelX before returning. It does not validate the final PeepPkg header layout, chunk table, CRC/checksum, compatibility schema, signatures, copy/install commit, or runtime exposure.
- FW0 provides a volatile development-load bridge after the minimum envelope prompt. Pressing `A` asks `thStorage` to reopen the single staged `.egg`, reject files outside the fixed `65536`-byte RAM capacity, read the complete file into an unpublished staging buffer, verify `PKG1` plus the header-declared file size, and close the file, FileX media, and LevelX before publishing the completed RAM image. A failed or partial load invalidates the RAM image and leaves the embedded development package as the default fallback. The package browser reports `LOADED RAM`; this is not a persistent install.
- `thStorage` owns the FileX read and RAM publication. It does not use HASH or decode package schemas. After storage is closed, `thRuntime` resolves the immutable RAM view and performs the existing SHA-256, header CRC, chunk CRC, and schema validation before any scene becomes active. Runtime therefore never reads FAT/FileX, and malformed staged bytes cannot become an active package.
- The volatile bridge uses one fixed RAM image. A staged package must be exited before `thStorage` begins replacing that image; installer admission provides that lifetime boundary. Persistent install, raw package regions, package index updates, and atomic flash commit remain future work.
- HW6 evidence `EV-HW6-20260813-P1-PKGMSC-043` validates the earlier package-page force-rescan path: with `dirty=0` and `force=1`, reclaim scanned staging, classified one package file plus one host metadata directory as `PACKAGE_CANDIDATE`, and validated the `PKG1` minimum envelope. HW6 evidence `EV-HW6-20260813-P1-RUNTIME-044` validates the runtime-host installer overlay around that earlier scaffold; it does not validate the new full RAM load bridge or persistent install.
...
10. firmware reclaim/rescan detects and classifies changed staging contents; package-page MSC reclaim also force-scans current staging contents for the import scaffold even when the bridge dirty flag is false
