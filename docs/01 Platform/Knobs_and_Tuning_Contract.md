...
HW6 FW0 currently uses this pipeline in `firmware/peepshow_hw6_fw0` for the START shipping-prep scaffold, battery power-policy scaffold, conservative ADP5360 charger boot profile, clock-policy scaffolding, NINA/BLE command transport, RTOS owner stack budgets, input diagnostics, and TraceX/SWO bring-up visibility scaffolds. The target-local source files are `config/knobs.json`, `config/knobs.schema.json`, and `tools/gen_knobs.py`; the generated firmware output is `Core/Inc/knobs_autogen.h`. Current generated knobs cover START hold thresholds, START live-level stable-sample acceptance, TMAG3001 raw XYZ diagnostic capture sizing/timing (`KNOB_INPUT_JOYSTICK_XYZ_CAPTURE_SAMPLES`, `KNOB_INPUT_JOYSTICK_XYZ_REST_SAMPLES`, `KNOB_INPUT_JOYSTICK_XYZ_SWEEP_SAMPLES`, `KNOB_INPUT_JOYSTICK_XYZ_CAPTURE_PERIOD_MS`, and `KNOB_INPUT_JOYSTICK_XYZ_CAPTURE_TIMEOUT_MS`), the protected default-off `KNOB_POWER_START_SOFTWARE_SHIP_ENABLE` switch, battery monitor cadence, provisional warning/critical/restart-allow voltage thresholds, protected default-off critical/boot low-battery software-shipment gates, the boot-applied charger profile registers `KNOB_POWER_CHARGER_VBUS_ILIM`, `KNOB_POWER_CHARGER_TERMINATION_SETTING`, `KNOB_POWER_CHARGER_CURRENT_SETTING`, `KNOB_POWER_CHARGER_FUNCTION_SETTING`, and `KNOB_POWER_CHARGER_THERMISTOR_CONTROL`, power clock profile knobs including `KNOB_POWER_CLOCK_IO_HIGH_HZ` and `KNOB_POWER_CLOCK_PLL2_AUTOGATE_ENABLE`, default-off automatic STOP2 idle knobs `KNOB_POWER_AUTO_STOP2_ENABLE`, `KNOB_POWER_AUTO_STOP2_CHECK_PERIOD_MS`, `KNOB_POWER_AUTO_STOP2_IDLE_MS`, and the display wait backend selector `KNOB_POWER_STOP2_DISPLAY_WAIT_BACKEND`, NINA/BLE transport knobs `KNOB_COMM_BLE_RESET_ASSERT_MS`, `KNOB_COMM_BLE_BOOT_WAIT_MS`, `KNOB_COMM_BLE_BOOT_DRAIN_MS`, `KNOB_COMM_BLE_RX_WINDOW_MS`, `KNOB_COMM_BLE_RX_QUIET_MS`, `KNOB_COMM_BLE_STOP_SETTLE_MS`, `KNOB_COMM_BLE_WAKE_SETTLE_MS`, `KNOB_COMM_BLE_RX_BYTE_TIMEOUT_MS`, `KNOB_COMM_BLE_TX_TIMEOUT_MS`, and `KNOB_COMM_BLE_RX_BUFFER_BYTES`, RTOS stack-budget knobs `KNOB_RTOS_DEFAULT_STACK_BYTES`, `KNOB_RTOS_POWER_STACK_BYTES`, `KNOB_RTOS_INPUT_STACK_BYTES`, and `KNOB_RTOS_STORAGE_STACK_BYTES`, and debug visibility knobs including `KNOB_DEBUG_TRACEX_ENABLE`, `KNOB_DEBUG_TRACEX_USER_EVENTS_ENABLE`, `KNOB_DEBUG_SWO_LIFECYCLE_ENABLE`, `KNOB_DEBUG_TRACEX_BUFFER_BYTES`, and `KNOB_DEBUG_TRACEX_REGISTRY_ENTRIES`. Charger profile knobs are power-owner boot-applied hardware policy values; RTOS stack knobs, BLE transport knobs, input capture knobs, and TraceX buffer knobs are kernel/debug static-budget values; normal package tools must not expose these groups, and promotion from retained conservative defaults remains evidence-gated.

The HW6 FW0 runtime owner has a dedicated `KNOB_RTOS_RUNTIME_STACK_BYTES` budget because package hashing, graph decoding, and scene execution require more stack than the default owner path.
...
| `platform.knobs.rtos.*` | ThreadX owner stack budgets and RTOS resource sizing |
