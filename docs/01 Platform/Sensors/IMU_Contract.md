...
This document defines the Platform contract for the retained HW6 IMU. Electrical behavior, low-power mode, and wake behavior remain pending HW6 revalidation.
...
The exact MCU sleep class, IMU register mode, and wake path must be proven on HW6 and recorded in [[HW6_Brought_Up_Tracker]].
...
STOP2 resident policy preserves the logical active IMU mode while selecting the physical state allowed during sleep. The default STOP2 target is `OFF`; FW0 must either preserve a current LIS2DUX12 deep-power-down proof or run the terminal deep-power-down write as the final LIS transaction, then omit post-sleep reads because later I2C/I3C traffic can wake the device from deep power-down. In the baseline STOP2 sequence, `thSensor` parks LIS before `thInput` parks TMAG so the TMAG sleep write remains the final I2C3 transaction before STOP2 entry. `STEP_COUNTER` is the intended future STOP2-resident mode because it may keep embedded step counting alive while the MCU sleeps, but FW0 must block STOP2 for that target until the LIS2DUX12 register sequence, retention behavior, and interrupt policy are target-validated; step-counter residency must not reuse the deep-power-down terminal park because deep power-down resets LIS registers and embedded state. `LOW_RATE_SAMPLE`, `EVENT_ARMED`, and `STREAMING` are not valid STOP2 resident states unless a later measured policy explicitly promotes them.
