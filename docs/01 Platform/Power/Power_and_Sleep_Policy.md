...
STOP2 entry boundaries are now split into explicit decision and entry steps. `thPower` first runs the STOP2 eligibility dry-run and records the blocker mask without sleeping; the default-off controlled-entry helper may enter STOP2 only when that ledger is clear, then reuses the validated owner-quiesce, STOP2, physical START wake, clock-restore, and owner-recovery scaffold. If any hard blocker is present, the helper refuses entry and records the blocker instead of sleeping.
