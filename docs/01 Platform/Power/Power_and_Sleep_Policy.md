...
4. Audio playback requests `SAI_AUDIO_ACTIVE`: SAI kernel clocks remain stable for the sample rate, audio DMA is stopped before sleep, and CPU/SYSCLK policy must not disturb the audio clock. After STOP2, `thPower` restores and verifies the active voltage scale before re-arming PLL2P, handing the SAI mux back to PLL2P, and granting the requester. `thAudio` owns playback and DMA state but does not write PWR, RCC, PLL2, SAI mux, or SAI clock/reset controls.
...
The no-sound audio requester named in the first-implementation inventory above is historical. Current HW6 evidence extends that same ownership boundary through audible package-backed SFX and post-STOP replay. `thPower` owns the complete physical sequence: voltage-scale restoration and readiness, PLL2 epoch invalidation/re-arm, PLL2P output ownership, SAI mux park/restore, and SAI clock/reset gating. `thAudio` receives only a bounded capability result, then owns SAI data-path and DMA operation until it drains and releases the capability.
...
HW6 evidence `EV-HW6-20260814-P1-STOP2STORAGE-064` validates STOP2 wake with storage attached and flash parked in deep power-down. After non-destructive attach, controlled STOP2 entry reported status `0x0`, owner STOP2 count `0 -> 1`, START wake source/primary `0x1/1`, and owner quiesce/enter/clock/recover/last all `0x0`. The post-STOP storage path requested and released `OCTOSPI_ACTIVE` internally from `thPower` (`storage clock post/release/status = 0x0/0x0/0x0`) before flash revalidation; storage power release and JEDEC checks both returned `0x0`, JEDEC match was `1`, and OCTOSPI clocks were restored for the flash check then parked again. Current HW6 staged-package evidence extends that rule to the opposite boundary: STOP2 quiesce temporarily acquires `OCTOSPI_ACTIVE` before asking storage to place an awake flash into deep power-down, then releases it after the command completes. Four consecutive staged-package STOP2 cycles reported quiesce/enter/clock/recover `0x0/0x0/0x0/0x0`, complete owner masks with failure mask `0x0`, and successful storage clock cleanup. Together these captures validate clock ownership around both physical flash wake and park operations; they do not validate persistent package installation or its interruption recovery.
...
HW6 evidence `EV-HW6-20260821-P1-LPBAMRESUME-083` validates phase-preserving wake from an admitted LPBAM program. After STOP2 returns, `thPower` asks `thDisplay` to snapshot autonomous progress before hardware reclaim. The validated capture mapped a parked LPDMA node to sequence/frame/phase `0/1/1`, measured LPTIM `6589/7814`, rendered the matching awake phase successfully, and scheduled the next software edge after the calculated four remaining ThreadX ticks. Probe APIs `53/28` reported resume/snapshot/render/abort status `0x0`, three successful STOP2 entries, input enqueue/dequeue/drop `46/46/0`, and logical/policy delivery `23/23`; the user reported correct visible behavior. Pre-entry cancellation still uses the conservative solid-frame reset path, while only a completed STOP2 wake preserves the autonomous timeline. This evidence covers the DMA-waiting state; wake during an active transaction remains to be captured separately.

HW6 evidence `EV-HW6-20260821-P1-LPBAMLAYOUT-084` validates the fixed SRAM4 animation budget and removal of the correctness-oriented quiet window. The permanent payload arena is `10512` bytes in `18` equal slots; six `28`-row transactions cover one panel state, guaranteeing three full-panel states. A black/white/checker target program used all `18` slots, `10356` wire bytes, and `126` queue nodes and played as coherent whole-panel states. Automatic admission now checks every `100 ms`, configures no additional idle delay, and permits only one LPBAM edge handoff at a time. Target stress processed raw edges `262/262/0` send/process/drop, queue operations `262/262/0` enqueue/dequeue/drop, and logical/policy/UI actions `131/131/131`, while completing `15` STOP2 entries with owner status `0x0`. The final atomic input snapshots also converged (`206/206`, queue empty, status `0x0`). This proves that ordered input delivery and the pre-WFI veto protect input across STOP2 without relying on a quiet period. The remaining observed wake residence is primarily the selected phase-edge wait plus bounded admission/display work; it is not an input blind interval.

HW6 evidence `EV-HW6-20260822-P1-LPBAMPHASE-085` extends phase-preserving entry and wake from the two-state cursor to a combined two-phase/three-phase program. Probe APIs `54/30` compiled the preferred six-step timeline without fallback, retained exact sequence position through STOP2 wake, mapped the displayed autonomous frame back to preferred frame `0`, and resumed awake scheduling at frame/count `0/6` with successful mapping and resume status. Eight STOP2 entries completed, and the user confirmed that the three-phase element remained visually ordered as `1,2,3` on every stop. STOP2 admission therefore carries one combined sequence position and cadence deadline rather than reducing a scene to cursor visibility; the static scene used for recompilation also excludes transient animation overlays.

HW6 evidence `EV-HW6-20260822-P1-LPBAMGUARANTEED-086` validates bounded resource fallback without extending awake residence or discarding the preferred timeline. A four-state full-panel preferred scene reached `18/18` transactions and `10512/10512` payload bytes after three states, rejected with `CHUNKS`, and triggered exactly one guaranteed compile. That retry admitted three global states `0/1/2`, played coherent black/white/checker frames in STOP2, and completed repeated wake/re-entry. Wake mapped the live guaranteed state into the corresponding frame of the four-state preferred descriptor with mapping/resume status `0x0/0x0`; a temporary target diagnostic also physically presented omitted preferred phase `3` after wake. The diagnostic timing override was then removed and the cleaned RTOS/owner API `54/31` firmware passed the normal cursor/input/STOP2 baseline. Resource pressure therefore reduces only autonomous presentation to the deterministic three-state policy; it does not keep the CPU awake, partially arm an over-budget queue, or permanently truncate the reactive scene timeline.
...
- `CONTINUOUS`: PeepOS does not apply an automatic inactivity timeout; normal admitted wake sources and package input remain available.
...
- `CONTINUOUS` disables only automatic timeout; PeepOS does not impose a manual-INACTIVE gesture
- a `CONTINUOUS` package does not require an authored inactive route or inactivity deferral
- while `INACTIVE`, only target/system-admitted activation gestures restore normal package interaction
- an author-declared inactive-exit binding is the intended replacement; Platform will emit generic input and will not reserve a chord such as `L+R`, but that package/Studio contract is not implemented yet
...
- while `ACTIVE`, a `START` release before the system-menu threshold is a normal package short-press input; reaching that threshold consumes the gesture and opens PeepOS without delivering package `START`
- continued `START` hold after the system menu opens remains owned by the shipping-prep, warning, imminent, panel-clear, and hardware-shipping progression
...
- after the cue expires, PeepOS restores the inactive waiting presentation and returns to the deepest admitted sleep state; short `START` activates existing legacy packages until declared inactive-exit bindings are available
...
- joystick movement wake is disarmed while `INACTIVE` under the initial HW6 `TIMEOUT` policy
...
- the default package interaction mode and supported interaction modes
- the system inactivity timeout used by `TIMEOUT` packages
- the bounded inactive-input cue duration and admitted cue buttons
