...
Phase alignment remains explicitly open. The validated run prepared the LPBAM list at tick `793` and committed/WFI at tick `818`, a full `25`-tick owner heartbeat later. Both animation engines are nominally `250 ms`; the mismatch is the scheduling gap because a display-side update to `stop2_auto_next_tick` does not wake `thPower` while it waits on its queue. The next power-policy change must provide a bounded display-to-power wake/nudge after successful prearm so commit can occur near the intended blink edge. The validated `4 MHz` MSIK, LPTIM `/128`, `ARR/CMP=7812/3906`, and normal APB3 `/8` policy must remain unchanged unless new target evidence contradicts them.
