...
- SPI3 remains at the normal `2 MHz` wire rate from the `4 MHz` MSIK kernel and `/2` prescaler. Its FIFO threshold is four 8-bit frames, matching each 32-bit DMA body write; reverting to a one-frame threshold recreates FIFO starvation in STOP2.
...
Known phase-alignment limitation at this checkpoint:
...
- The successful target run prepared LPBAM at tick `793` but committed it at tick `818`, exactly one `25`-tick owner heartbeat later. Updating `stop2_auto_next_tick` does not wake a power thread blocked in its queue receive, and the first LPBAM frame remains LPTIM-triggered.
- Therefore the next timing change must wake `thPower` when `thDisplay` prearms LPBAM and commit close to the selected blink edge. It must not compensate by changing the already-correct LPTIM clock source, prescaler, or nominal frame period.
