...
- FW0 now has a target-proven LPBAM cursor-slice handoff that can arm SPI3,
  ...
  intervention, wake, and reclaim normal display ownership; the complete cursor
  now changes in unison, but the current payload is still a bring-up slice rather
  than the final renderer-driven waiting-animation compiler
