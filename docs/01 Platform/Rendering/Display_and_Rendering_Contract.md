...
LPBAM display animation is a hardware-validated HW5 Platform baseline for eligible reactive waiting visuals. On HW6 the owner handoff, SRAM4 placement, linked-list start, STOP2 residency, complete-cursor row-unison behavior, wake, and abort/reclaim path are now validated for the current cursor-slice bring-up payload. Shipping use remains target-profile gated until phase-aligned handoff, general waiting-animation compilation, SRAM4 admission limits, LPBAM active current, and display electrical behavior are evidenced for production waiting visuals.
...
- This cursor-region compiler is not the final general animation compiler. FW0 has a renderer-owned panel-native dirty-row contract, normal display presentation consumes that row list through `LCD_PresentRows_DMA()`, and HOME cursor movement/blinking updates only rows `153..160`. LPBAM currently derives the same bounded cursor region from renderer animation intent and compares consecutive compiled frames within it; later waiting-animation compilation must generalize this without bypassing renderer-owned dirty state.
