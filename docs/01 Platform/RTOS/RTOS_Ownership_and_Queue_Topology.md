...
The same evidence exposes one scheduling issue for the next checkpoint: LPBAM prepare completed at tick `793`, but `thPower` did not commit until tick `818`. All owners currently use a `25`-tick queue heartbeat, and the display-side request only advances `stop2_auto_next_tick`; it does not wake a power owner already blocked in `tx_queue_receive`. The next phase-alignment change must add a bounded wake/nudge through the existing owner boundary, not call STOP2 entry from `thDisplay` and not retune the validated LPTIM clock configuration.
