...
The required knob names above are authoritative. HW6 FW0 now sources the START overlay scaffold values from `firmware/peepshow_hw6_fw0/config/knobs.json`, validates them with `config/knobs.schema.json`, generates `Core/Inc/knobs_autogen.h` through `tools/gen_knobs.py`, and consumes only generated macros. The current generated defaults preserve the validated scaffold shape while allowing timing adjustment: START long press `1000 ms`, ship-prep currently `6000 ms`, warning `9000 ms`, imminent `11000 ms`, START stable-level acceptance `2` samples, and software shipment request enable `false`. Earlier HW6 unit 001 evidence used the prior `5000 ms` prep value.
