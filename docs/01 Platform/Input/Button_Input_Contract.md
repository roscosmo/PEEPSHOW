...
This is not the complete button contract. Chord publication, stuck-event
publication, non-START wake-source current measurement, and `BTN_BOOT` remain
open. A/B/L/R hold/repeat publication is implemented but requires the current
STATE lifecycle target proof before it becomes measured HW6 evidence. START shipping-prep/warning/imminent timing has an FW0 scaffold validated
