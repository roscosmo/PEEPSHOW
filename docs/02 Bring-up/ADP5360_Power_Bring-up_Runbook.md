...
| boot low-battery restart gate | boot below restart-allow threshold with and without VBUS | no normal runtime below threshold without VBUS; VBUS permits charge/recovery shell | no-VBUS boot/restart block validated at PMIC-read `3270 mV`: policy/event `5/5`, boot gate pending/blocked/clear `1/1/0`, quiesce `1`, software ship request/skip `0/1`, power/PMIC `8/8`, UI/display page/shutdown `8/5`; recovery above restart threshold validated at `3707 mV`: policy/event `2/2`, boot gate `0/0/1`, power/PMIC `2/3`, UI/display `1/0` | boot_gate_recovery_pass_unit_001; VBUS recovery exception and enabled software shipment open |
