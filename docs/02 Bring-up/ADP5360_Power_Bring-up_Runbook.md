...
| boot low-battery restart gate | boot below restart-allow threshold with and without VBUS | no normal runtime below threshold without VBUS; VBUS permits charge/recovery shell | runtime recovery from critical path validated when source returned to `3.8 V` and PMIC read `3814 mV`, policy/event `2/2`, power/PMIC `2/3`; boot gate itself TBD | runtime_recovery_pass_unit_001; boot gate open |
