...
| external flash | B | command baseline | JEDEC/status, scratch erase/program/readback, deep-power-down | `partial_unit_001`; non-destructive storage-owner `ps_dev_at25sl128a` JEDEC/release/deep-power-down lifecycle passes with JEDEC `1f 42 18`, two cycle release/JEDEC/match/DPD results `0/0/1/0`, and OSPI state/error `0x2/0x0`; scratch erase/program/readback, DMA flash transfers, FileX/LevelX, USB MSC, timing, current, and fault injection remain pending |
...
| RTOS owner topology | B | code/queue design | owner creation, queue routing, quiesce acknowledgements, fault paths | `partial_unit_001`; all objects/startup envelopes plus real power-to-display/audio routing, bounded acknowledgements, serialized PMIC lease, sole-owner physical actions, and two lifecycle-v9 driver-backed wake/revalidate/quiesce cycles pass for LIS2DUX12, TMAG3001, display, audio, and non-destructive flash; saturation, fault injection, cancellation, STOP2 handoff, and production message contracts remain pending |
