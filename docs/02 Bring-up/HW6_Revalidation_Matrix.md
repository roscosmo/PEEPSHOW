...
| speaker audio | B | sample/DMA baseline | 16 kHz clock, SAI DMA, audible output, current, clean shutdown | `partial_unit_001`; driver-backed 4.096 MHz kernel, 16 kHz SAI DMA, package-backed multi-second playback across STOP2, and clean five-voice overlap at `80 MHz` and the audio-only `48 MHz` candidate are target-proven with zero refill underruns; music plus five SFX, strict higher-priority preemption, concurrent display margin, final energy/clock selection, and fault behavior remain pending |
...
| joystick / TMAG3001 | B | driver and threshold baseline | identity, calibration, axis mapping, wake interrupt, sleep current | `partial_unit_001`; persistent guided calibration, normalized vector, canonical diagonal detection, per-scene four/eight-way policy, deterministic four-way shell resolution, four logical cardinal events, X/Y omnipolar STOP2 wake for every cardinal, bounded post-wake direction confirmation, and matched five-minute `55 uA` disabled / `65 uA` enabled measurements are target-proven; neutral false-wake soak, hold/repeat policy, and PROGRAM vector polling remain pending |
...
| USB MSC / storage ownership | B | recovery procedure | attach/enumeration, mount/reclaim, package install/launch, disconnect recovery, power | `partial_unit_001`; host mount/read/write/delete, repeated export/reclaim, staging scan, persistent A/B package programming, commit-last index selection, bounded installed-resident publication, immediate and boot launch, selected-generation reload, and raw package-reader audio windows are target-proven; product single-slot migration, streamed MSC install, pre-commit full semantic validation, reset-stage injection, uninstall/quarantine, and final power/fault evidence remain pending |
