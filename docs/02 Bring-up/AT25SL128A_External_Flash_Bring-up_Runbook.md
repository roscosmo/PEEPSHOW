...
| USB export | staging/export only | host cannot see protected regions | `EV-HW6-20260809-P7-USB-019`: active USBX MSC export mounted a writable Windows FAT volume over the `USB_STAGING` FileX/LevelX media; user read expected `HW6_FXLX.TXT` payload, bridge submit/done/timeout/busy `199/199/0/0`, read/write/status callbacks `145/54/249`, dirty `1`, LUN `10239/512`, and `SystemCoreClock=160000000`; protected regions are not directly exposed by the fixed staging-only LUN | pass_partial_reclaim_open |
...
`EV-HW6-20260809-P7-USB-019` validates active HW6 USBX MSC host export on `HW6-UNIT-001`: Windows mounted the FAT staging volume, the user read the expected `HW6_FXLX.TXT` payload, and host traffic exercised both read and write callbacks through `thStorage`. The halted-state print reported export policy/DCD/init/start `0x0 / 0x0 / 0x0 / 0x0`, flash wake/FileX-LevelX open `0x0 / 0x0`, `SystemCoreClock=160000000`, `SysTick_LOAD=0x1869ff`, bridge submit/done/timeout/busy `199 / 199 / 0 / 0`, callbacks read/write/status `145 / 54 / 249`, dirty `1`, no DCD IRQ guard drops, configured interface class `0x08/0x06/0x50`, and LUN `10239 / 512`. This closes active host export and mounted-volume read/write for the staging LUN; USB reclaim, reconnect soak, package installer flow, and injected storage fault recovery remain open.
