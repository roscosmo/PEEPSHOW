...
| scratch range | final 4 KiB sector, `0x00FFF000..0x00FFFFFF` | bring-up-only destructive scratch region; future flash-layout pass must not assume this remains available | User approved use of the final sector for immediate bring-up tests before final protected/fault-log layout is assigned; `EV-HW6-20260802-P5-FLASH-011` touched the first `256` bytes and left them erased | active |
...
| DMA readback | scratch pattern | byte-for-byte match and clean completion signaling | HW5 256-byte GPDMA read of programmed scratch pattern passed after enabling OCTOSPI1 IRQ completion handling; HW6 owner-routed DMA flash transfer remains pending | HW5 pass / HW6 open |
| DMA program | scratch page pattern | page program completes and readback matches | HW5 256-byte page program via GPDMA passed after enabling OCTOSPI1 IRQ completion handling; HW6 owner-routed DMA flash transfer remains pending | HW5 pass / HW6 open |
...
This evidence closes the HW6 owner-routed polling scratch erase/program/readback baseline. It does not close DMA flash transfers, LevelX/FileX, USB MSC export/reclaim, protected fault-log layout, package installation, storage timing budgets, current, or storage fault injection/recovery.
