...
| PMC-MEM-006 | package runtime RAM limit | Engine/Memory | [[Target_Profile_Schema_Contract]], [[Package_Contract]] | map/budget report | Provisional | current FW0 whole-package activation cache is fixed at `65536` bytes and is target-proven for the STATE slice; this is a bring-up implementation ceiling, not the intended package-facing production limit |
...
| PMC-USB-001 | MSC mount/reclaim timing | USB/Storage | [[Storage_and_Installer_Contract]], [[USB_MSC_Bring-up_and_Recovery_Runbook]] | USB MSC runbook | Partial | host create/read/delete, 8-cycle reconnect/export/reclaim, forced package rescan, persistent A/B install, commit-last index selection, installed-RAM publication, immediate STATE launch, and post-install STOP2 recovery are target-proven. Precise timing, full pre-commit semantic validation, reset-stage injection, boot activation, uninstall/fallback, and broader fault injection remain pending |
