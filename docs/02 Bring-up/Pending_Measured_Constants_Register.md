...
| PMC-POWER-004 | owner quiesce timeout values | Power/RTOS | [[Subsystem_State_Machines]], [[RTOS_Ownership_and_Queue_Topology]] | START shutdown barrier, sleep/wake telemetry, Tracealyzer/SWO evidence | Partial | FW0 START shutdown prep, pre-STOP sleep-prep, and manual STOP2 START-wake all validate the bounded owner-ACK barrier with required/send/ACK/success/failure masks `0x7e/0x7e/0x7e/0x7e/0x0` and all owner statuses `0x0`; sleep-prep validates active-LP recovery with STOP intentionally skipped, and STOP2-035 validates real STOP2 entry/START wake/clock restore/power-FSM recovery; final timeout values, active-owner resume work, current, and fault-injection evidence remain pending |
