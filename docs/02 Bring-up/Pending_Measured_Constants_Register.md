...
|---|---|---|---|---|---|---|
| PMC-POWER-001 | steady waiting current per physical backend and armed context | Power | [[Power_and_Sleep_Policy]], [[Target_Profile_Schema_Contract]] | [[Sleep_Wake_Integration_Bring-up_Runbook]], power measurement artifacts | Measured | instrumentation-minimized five-minute means on unit 001 include `51.39 uA` held frame, approximately `56 uA` sparse 250 ms LPBAM, `85 uA` three-full-panel-state LPBAM, `55 uA` with joystick W&S disabled, and `65 uA` with joystick W&S enabled; production-scene matrix and profile adoption remain open |
| PMC-POWER-002 | wake latency per wake source | Power/Input/Sensor | [[Power_and_Sleep_Policy]], [[Input_Focus_API_Contract]], [[Target_Profile_Schema_Contract]] | [[Sleep_Wake_Integration_Bring-up_Runbook]] | Partial | joystick movement wake reached capture-to-logical delivery in `3` ThreadX ticks after the fast-path reorder, while START and A/B/L/R wake/delivery are functionally proven; comparable RTC, button, IMU, PMIC, and USB latency captures remain open |
...
| PMC-POWER-004 | owner quiesce timeout values | Power/RTOS | [[Subsystem_State_Machines]], [[RTOS_Ownership_and_Queue_Topology]] | START shutdown barrier, sleep/wake telemetry, Tracealyzer/SWO evidence | Partial | FW0 START shutdown prep, pre-STOP sleep-prep, manual STOP2 START-wake, STOP2 eligibility dry-run, and default-off controlled STOP2 entry validate the bounded owner-ACK/admission path with required/send/ACK/success/failure masks `0x7e/0x7e/0x7e/0x7e/0x0` and all owner statuses `0x0`; sleep-prep validates active-LP recovery with STOP intentionally skipped, STOP2-035 validates real STOP2 entry/START wake/clock restore/power-FSM recovery, probe API `27` validates staged active-owner resume/quiesce plus post-wake liveness for audio/input/display/sensor/comm with storage/flash intentionally excluded, and STOP2CTRL-053 validates that controlled real STOP2 entry is gated by a clear eligibility ledger; final timeout values, current, repeated cycles, and fault-injection evidence remain pending |
| PMC-POWER-005 | mandatory system inactivity timeout/default/min/max | Power/Engine | [[Time_And_Power_Intent_API_Contract]], [[Target_Profile_Schema_Contract]] | integration testing and UX review | Provisional | packages cannot disable system inactivity handling or author the timeout; final system default and bounds remain evidence/policy gated |
...
| PMC-POWER-008 | waiting-visual autonomous cadence | Power/Display | [[Target_Profile_Schema_Contract]], [[Digital_Twin_Host_Runtime_Contract]] | [[LPBAM_Autonomous_Display_Validation_Plan]] | Measured | HW6 repeatedly runs the current authored waiting timeline at the fixed `250 ms` quantum awake and in LPBAM STOP2; profile adoption remains pending until the complete target limit set is frozen |
...
| PMC-POWER-012 | STATE/reactive active operating-point selection | Power/Clock/Runtime/Audio | [[Power_and_Sleep_Policy]], [[HW6_Clock_Tree_Contract]], [[Power_Validation]] | deterministic reactive workload sweep with PPK2 plus bounded timing/trace evidence | Pending | choose the transaction-energy minimum subject to response-latency and correctness limits for logic-only state changes, retained-element updates, text and Tiled-map region composition, full/partial display and waiting-program preparation, installed asset reads, bounded STATE SFX decode/refill, and one combined input/render/display/SFX transaction; verify return to STOP2 and keep SAI/OCTOSPI kernel requirements independent from CPU profile choice |
...
| PMC-DISPLAY-005 | SRAM4 DMA reachability for display payloads | Display/Memory | [[Memory_and_Budgeting_Contract]], [[HW6_DMA_Map]] | DMA/display flush evidence | Measured | repeated automatic STOP2 playback proves LPDMA access to retained SRAM4 payloads and descriptors; awake display staging is intentionally in normal SRAM through GPDMA |
| PMC-DISPLAY-006 | waiting-visual animation support through LPBAM | Display/Power | [[Target_Profile_Schema_Contract]], [[Digital_Twin_Host_Runtime_Contract]] | [[LPBAM_Autonomous_Display_Validation_Plan]], [[HW6_Revalidation_Matrix]] | Measured | mixed 2/3-phase renderer-owned waiting visuals, coherent multi-chunk frames, deterministic three-state fallback, phase-preserving wake/resume, and repeated STOP2 entry are target-proven; shipping-profile promotion and fault injection remain open |
| PMC-DISPLAY-007 | compiled waiting-visual slice payload/chunk/changed-row limits | Display/Memory | [[Display_and_Rendering_Contract]], [[Memory_and_Budgeting_Contract]] | [[LPBAM_Autonomous_Display_Validation_Plan]] | Measured | HW6 uses a fixed `10512`-byte payload arena in `18` equal slots; six 28-row transactions cover one full panel state, guaranteeing three full-panel autonomous states; preferred programs may use up to 12 combined timeline steps but fall back deterministically when resources exceed the admitted set |
| PMC-MEM-001 | SRAM4 display-DMA/autonomous arena partition | Memory/Display | [[Memory_and_Budgeting_Contract]], linker script | [[LPBAM_Autonomous_Display_Validation_Plan]], map file and SRAM4 DMA evidence | Measured | fixed HW6 LPBAM allocation uses `10512` payload bytes in `18` equal slots inside retained SRAM4; three full-panel states and repeated LPDMA execution are target-proven; all SRAM banks remain powered in STOP2 unless a later measured optimization and STM32U575/U585 erratum review changes policy |
...
| PMC-MEM-006 | package resident-prefix RAM limit | Engine/Memory | [[Target_Profile_Schema_Contract]], [[Package_Contract]] | map/budget report | Provisional | current FW0 resident metadata/pixel prefix is fixed at `65536` bytes; packages may be larger when approved nonresident audio tails use bounded storage-owner reader windows. Generalized large-asset coverage and the final production resident budget remain pending |
| PMC-MEM-007 | package asset/blob size limits | Assets/Storage | [[Package_Blob_Format_Contract]], [[Target_Profile_Schema_Contract]] | storage/package validation | Partial | persistent package slots are `5 MiB` each and atomic install is target-proven for a `2168`-byte `.egg`; final package/asset limits require the bounded installed-asset API, RAM/cache budgets, timing, and validation evidence |
...
| PMC-INPUT-005 | joystick center/deadzone/hysteresis | Input/Sensor | [[Joystick_Hall_Input_Contract]], [[Input_Focus_API_Contract]] | [[TMAG3001_Joystick_Bring-up_Runbook]] | Measured | guided calibration captures neutral/cardinals/full sweep, stores per-unit transform/extents persistently, and target-proves normalized output with current direction enter/release/dominance values `450/300/100`; packages see logical directions or normalized vectors only, never these physical values |
| PMC-INPUT-006 | joystick wake threshold and polarity | Input/Power | [[HW6_Wake_Sources]], [[Power_and_Sleep_Policy]] | joystick + sleep runbooks | Measured | HW6 uses active-low `JOY_INT` with TMAG3001 X/Y omnipolar switch mode, field threshold `48`, hysteresis `7`, and `20 ms` sensor period; all four cardinal wakes and approximately `10 uA` incremental STOP2 cost are target-proven; long neutral false-wake soak remains open |
| PMC-POWER-020 | START software shipment enable policy | Power/Input | [[PMIC_and_Power_Contract]], [[Button_Input_Contract]], Platform knobs | ADP5360 power runbook | Partial | `KNOB_POWER_START_SOFTWARE_SHIP_ENABLE` added with generated default `false`; default-off gate target-validated with enable/request/skip `0/0/1` and PMIC sw ship count `0`; START prep now validates owner-ACK quiesce and PMIC ship-pending persistence; when enabled, START imminent can request ADP5360 `0x36 = 1`; enabled START register-entry validation and final product timing remain pending |
...
| PMC-SENSOR-005 | IMU motion stream context limits | Sensor/Runtime | [[Sensor_API_Contract]], [[Target_Profile_Schema_Contract]] | IMU + power tests | Pending | sample rate, event rate, duration, wake behavior, and scene-type validity |
...
| PMC-STORAGE-001 | AT25SL128A JEDEC/device ID and status meanings | Storage | [[Storage_and_Installer_Contract]] | [[AT25SL128A_External_Flash_Bring-up_Runbook]] | Measured | HW6 reads JEDEC `1f 42 18`; owner-routed probe, release, status, program/erase/readback, and deep-power-down behavior are target-proven |
...
| PMC-USB-001 | MSC mount/reclaim timing | USB/Storage | [[Storage_and_Installer_Contract]], [[USB_MSC_Bring-up_and_Recovery_Runbook]] | USB MSC runbook | Partial | host create/read/delete, 8-cycle reconnect/export/reclaim, explicit package scan, persistent A/B install, commit-last index selection, bounded installed-resident publication, immediate and boot STATE launch, raw package-reader audio windows, and post-install STOP2 recovery are target-proven. Precise timing, product single-slot migration, streamed MSC install, full pre-commit semantic validation, reset-stage injection, uninstall/quarantine, and broader fault injection remain pending |
