...
- Status: `owner_lifecycle_v13_usbx_msc_export_policy_passed`; all seven physical owner domains reached
...
- Active FW0 ELF SHA-256: `C830E39787B3A7287CCF342EAD58ECBDC6DE45348E90FEACCCE29C0DC117DE84`
...
| 2 - display and storage | partial: AT25SL128A identity/status, storage-owned driver-backed JEDEC/release/deep-power-down lifecycle, owner-routed polling scratch erase/program/readback/cleanup, owner-routed DMA flash program/readback, raw 4 KiB block-adapter scratch erase/program/readback/cleanup, reset-normalized AT25 write/erase operation, fixed 10-region flash layout validation, local FileX/LevelX format/mount/create/write/read smoke over `USB_STAGING`, CubeMX-generated USBX Device MSC baseline non-regression with USB still parked, and RTOS-owned driver-backed full-frame SPI3/LPDMA diagnostic display pass; partial updates, package install, USB MSC export/reclaim, and storage fault injection remain open | display/flash runbooks, [[HW6_DMA_Map]] | retained display plus polling, DMA, raw block flash identity/deep-power-down/scratch behavior, flash layout validation, and local FileX/LevelX smoke behavior proven |
...
| 7 - USB, BLE, NFC, installer | partial: NINA-B112 bounded AT handshake and owner-routed `AT&D4`/DSR STOP entry pass; detached USB shutdown passes; CubeMX-generated USBX Device MSC, bounded `thStorage` MSC bridge, and default-closed MSC export policy initialize with correct 1 MiB staging geometry while USB remains parked; BLE data/NFC/power, USB host attach/export/reclaim, resume, and connected USB behavior remain open | communication/USB runbooks | transport, recovery, sessions, NFC, and power proven |
