...
- Reclaim must return ownership to firmware, close the host-export filesystem path, release the USB clock capability, and let `thStorage` classify dirty staging/export contents before normal runtime resumes. The FW0 package-page import scaffold may force this classification on reclaim so the current staging contents are inspected even if no fresh write was observed during that exact export session.
- FW0 package-candidate handling remains a reclaim-time storage result, not a USB personality. If dirty reclaim or package-page forced reclaim finds exactly one `.egg` file and no unsupported files, `thStorage` runs a read-only minimum-envelope validator that requires the first four bytes to be `PKG1`. A valid minimum envelope may request a package-valid UI prompt; a failed validator may request a package-error prompt. After user confirmation, the current development install action asks `thStorage` to copy one complete, bounded `.egg` into the fixed `65536`-byte runtime buffer, atomically program and byte-verify the inactive A/B package slot, program and verify the inactive index body, and program its `CMIT` marker last. It then parks flash, publishes that selected generation as installed runtime RAM, and queues immediate STATE scene activation. `thRuntime` performs SHA-256, container, chunk, and scene-schema validation before the UI changes `INSTALLING` to `INSTALLED` and enters runtime. Pre-commit SHA-256 and full container validation remain required production hardening under [[Storage_and_Installer_Contract]].
