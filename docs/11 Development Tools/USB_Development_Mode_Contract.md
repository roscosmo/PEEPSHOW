...
- FW0 package-candidate handling remains a reclaim-time storage result, not a USB personality. If dirty reclaim or package-page forced reclaim finds exactly one `.egg` file and no unsupported files, `thStorage` runs a read-only minimum-envelope validator that requires the first four bytes to be `PKG1`. A valid minimum envelope may request a package-valid UI prompt; a failed validator may request a package-error prompt. The current development install action asks `thStorage` to copy one complete, bounded `.egg` into the fixed `65536`-byte staged-RAM source, close FileX and LevelX, park flash, and publish the immutable RAM image. `thRuntime` then performs the normal SHA-256, container, chunk, and scene-schema validation before activation. This bridge is volatile and is not the persistent package installer.
...
This personality is governed by [[Storage_and_Installer_Contract]] and [[USB_MSC_Bring-up_and_Recovery_Runbook]]. FW0 target evidence `EV-HW6-20260812-P1-CLOCKUSB-037` validates the current manual export/reclaim request path through `thStorage` and `thPower`; `EV-HW6-20260812-P1-CLOCKSTORAGE-039` validates the named storage clock requester wrapper and menu/service export/reclaim counters; `EV-HW6-20260812-P1-FLASHINIT-040` validates explicit destructive staging provisioning as a separate storage maintenance command; `EV-HW6-20260812-P1-MSCSMOKE-041` validates host create/read/delete smoke on the provisioned staging volume; `EV-HW6-20260812-P1-MSCSOAK-042` validates 8-cycle reconnect/export/reclaim stability; `EV-HW6-20260812-P1-CLOCKBOOT-038` separately validates normal-boot USB parking with no MSC exposure; `EV-HW6-20260813-P1-USBOOTFIX-049` validates inactive boot-park idempotence and active MSC export/reclaim non-regression; `EV-HW6-20260813-P1-PKGMSC-043` validates the earlier package-page force-rescan, minimum `PKG1` validator, install-stub action, and return-to-menu UI path; and `EV-HW6-20260813-P1-RUNTIME-044` validates the runtime-host view of that earlier scaffold as an `INSTALLER` overlay returning to `SHELL`. Current HW6 target evidence additionally validates the complete volatile staged-RAM load, runtime HASH/schema validation and activation, animated STATE scene execution, return to HOME, and repeated STOP2 entry after flash is parked. Persistent raw-blob commit, installed-package indexing, boot-time installed-source selection, and rollback remain separate work.
