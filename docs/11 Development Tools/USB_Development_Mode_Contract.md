...
- FW0 package-candidate handling remains a reclaim-time storage result, not a USB personality. If dirty reclaim or package-page forced reclaim finds exactly one `.peepkg` / `.ppkg` file and no unsupported files, `thStorage` runs a read-only minimum-envelope validator that requires the first four bytes to be `PKG1`. A valid minimum envelope may request a package-valid UI prompt; a failed validator may request a package-error prompt. The current install action is still a storage-owned no-op stub pending the real package validator and install schema.
