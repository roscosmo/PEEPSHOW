...
- MSC is transport only. Copying or ejecting files must not automatically install, launch, or publish package prompts. The package browser is the user-owned decision point for scanning available `.egg` files, choosing one, installing it into the A/B package store, launching an installed package, or deleting/quarantining files. The earlier reclaim-time single-package prompt remains bring-up evidence only, not product behavior.
