...
- Reclaim must return ownership to firmware, close the host-export filesystem path, release the USB clock capability, and let `thStorage` classify dirty staging/export contents before normal runtime resumes.
- FW0 package-candidate handling remains a reclaim-time storage result, not a USB personality. If a dirty reclaim finds exactly one `.peepkg` / `.ppkg` file and no unsupported files, `thStorage` may request a package-found UI prompt; the current install action is a storage-owned no-op stub pending the real package validator and install schema.
