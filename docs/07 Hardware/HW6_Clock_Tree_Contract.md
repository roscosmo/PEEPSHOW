...
| `CLK_REACTIVE_BURST` | bounded shell or STATE transaction where map composition, multi-element rendering, text, or waiting-program preparation makes racing back to STOP more efficient | upper MSI/MSIS class; FW0 implements `48 MHz` | no extra kernel clocks unless an owner independently requests them | implemented development candidate; measurement pending |
| `CLK_REALTIME_BALANCED` | runtime/gameplay with frame, input, sensor, display, and optional audio deadlines; also the lowest current mixer characterization point | PLL1 `80 MHz` candidate | SAI/OCTOSPI only when their owning threads request those peripherals | implemented development candidate; workload measurement pending |
| `CLK_REALTIME_HIGH` | intermediate compute point for workloads that miss deadlines at balanced without requiring the I/O ceiling | PLL1 `120 MHz` candidate | SAI/OCTOSPI only when their owning threads request those peripherals | implemented development candidate; workload measurement pending |
...
| `AUDIO_MIX_ACTIVE` | `thAudio` while one or more sampled voices own the mixer | hold one policy-selected CPU profile for the complete DMA/mix lifetime so overlapping owner requests cannot retune PLL1 during playback |
...
holds `SAI_AUDIO_ACTIVE`, `OCTOSPI_ACTIVE`, and `AUDIO_MIX_ACTIVE` so PLL2P and
the SAI sample clock remain stable while CPU decode/refill work uses one
Platform-selected point for the complete mix. The production default remains
`160 MHz`; development may select the implemented `120 MHz` or `80 MHz`
candidate between streams for characterization. Initial comparison keeps all
three PLL1 points at VOS1 so it changes CPU frequency without also changing
voltage. A later, separate measurement may qualify a lower voltage scale for an
eligible point. Packages cannot select a clock frequency.
