...
This note records expected HW6 rail controls and safe defaults. All electrical
values and current behavior remain `pending_validation` until arrival evidence.

| Rail / Control | Control | Boot Default | Waiting / STOP Behavior | Status |
|---|---|---|---|---|
| MCU/system rail | ADP5360 buck, expected 1.8 V baseline | hardware/PMIC defined | retained as required by selected sleep backend | pending measurement |
| 3.3 V peripheral/display rail | ADP5360 buck-boost | hardware/PMIC defined | remains enabled for the retained display architecture unless measured policy changes | pending measurement |
...
| PMIC/fuel/charger | ADP5360 over I2C3 | monitor after probe | may notify through `PMIC_INT` | pending measurement |
...
| Power debug marker | `PH1 PWR_DBG`, battery connector | low/idle | remains low unless an explicit development evidence procedure drives a timing marker | design resolved; electrical proof pending |
