...
| PMIC/fuel/charger | ADP5360 over I2C3 | monitor after probe | may notify through `PMIC_INT` | unit 001 full read map, no-fault state, and guarded six-byte reversible profile transaction proven; exact restore confirmed with PGOOD `0x07` and VBUS absent; current factory profile remains active, while fuel behavior, NTC/JEITA, VBUS limits, protection thresholds, persistent policy, charging, and IRQs require staged validation |
