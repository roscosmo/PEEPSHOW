...
app.whenReady().then(async()=>{
  ...
  await field('Animation name','Counter blink');
  ...
  assert.equal(clip.display_name,'Counter blink');
  ...
  await field('Animation name','Counter blink fast');await field('Animation cadence',800);await button('Apply clip');
  assert.equal(latest.document.animations[1].display_name,'Counter blink fast');
  ...
  assert(duplicate);
  assert.equal(duplicate.display_name,'Counter blink fast copy');
