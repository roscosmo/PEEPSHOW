export type SpriteImportConversionMode = "threshold_1bpp" | "ordered_2x2" | "ordered_4x4" | "floyd_steinberg";
export type SpriteImportAlphaMode = "respect_alpha" | "ignore_alpha" | "transparent_as_white";

export type SpriteImportConversion = {
  mode: SpriteImportConversionMode;
  threshold: number;
...
export type SpriteImportConversionResult = {
...
export const spriteImportOutputScale = (mode: SpriteImportConversionMode) => (
  mode === "ordered_2x2" ? 2 : mode === "ordered_4x4" ? 4 : 1
);
...
export function convertSpritePixels(
  source: Uint8ClampedArray,
  width: number,
  height: number,
  conversion: SpriteImportConversion,
): SpriteImportConversionResult {
  if (source.length !== width * height * 4) throw new Error("Sprite pixel buffer dimensions do not match.");
  const scale = spriteImportOutputScale(conversion.mode);
  const outputWidth = width * scale;
  const outputHeight = height * scale;
  const data = new Uint8ClampedArray(outputWidth * outputHeight * 4);
  const tones = new Float64Array(width * height);
  const transparent = new Uint8Array(width * height);
  const sourceColorKeys = new Set<number>();
  ...
  for (let pixel = 0; pixel < width * height; pixel += 1) {
    ...
    if (!sourceTransparent) {
      ...
      if (!sourceColorKeys.has(colorKey)) {
        if (sourceColorKeys.size < SOURCE_COLOR_LIMIT) sourceColorKeys.add(colorKey);
        else sourceColorsLimited = true;
      }
    }
  }
  ...
  const writeOutputPixel = (x: number, y: number, black: boolean, isTransparent: boolean) => {
  ...
  if (scale > 1) {
    ...
    for (let sourceY = 0; sourceY < height; sourceY += 1) {
      for (let sourceX = 0; sourceX < width; sourceX += 1) {
        ...
        if (conversion.invert) blackCells = cells - blackCells;
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      if (conversion.mode === "floyd_steinberg") {
        ...
        const diffuse = (targetX: number, targetY: number, weight: number) => {
          ...
          if (transparent[target] === 0) ditheredTones[target] += error * weight;
        };
        diffuse(x + 1, y, 7 / 16);
