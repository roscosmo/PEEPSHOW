...
export function AnimationClipEditor({ clip, frames, assets, scenes, disabled, onApply }: {
  ...
  const changed = JSON.stringify(steps) !== JSON.stringify(initial());
  const frameLabel = (id: string) => {
    ...
    return asset ? `${asset.display_name ?? asset.asset_id} / ${frame?.display_name ?? id}` : id;
  };
  ...
  return <section className="clip-editor" aria-label="Animation clip editor" onKeyDown={event => {
    ...
    <h4>{clip.animation_id}</h4>
    <p className="muted">Project clip: changes affect every reference.</p>
    ...
    <fieldset disabled={disabled}>
      ...
      {!valid && <p role="status">Choose available frames and whole durations from 1 to 60000 ms.</p>}
      <div className="clip-editor-actions">
        <button className="button primary" type="button" disabled={!valid || !changed} onClick={async () => {
          if (await onApply([{ kind: "animation.upsert", animation: { ...clip,
            frame_refs: steps.map(step => step.frame), frame_duration_ms: steps.map(step => Number(step.duration)) } }])) setOpen(false);
        }}>Apply clip</button>
