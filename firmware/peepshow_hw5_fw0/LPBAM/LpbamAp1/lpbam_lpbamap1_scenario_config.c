...
static void MX_Queue1_Q_DMA_Error_Callback(DMA_HandleTypeDef *hdma);
/* LPBAM DMA NVIC API */
...
static void MX_Queue1_Q_Link(DMA_HandleTypeDef *hdma)
{
  ...
  if (HAL_DMA_RegisterCallback(hdma, HAL_DMA_XFER_ERROR_CB_ID, MX_Queue1_Q_DMA_Error_Callback) != HAL_OK)
  {
    Error_Handler();
  }
  MX_DMA_NVIC_Config(hdma, 15, 0);
}
...
/**
  * @brief  Queue1 queue dma error callback
  * @retval None
  */
static void MX_Queue1_Q_DMA_Error_Callback(DMA_HandleTypeDef *hdma)
{
  /* USER CODE BEGIN Queue1_DMA_Error_Callback */

  /* USER CODE END Queue1_DMA_Error_Callback */
}
