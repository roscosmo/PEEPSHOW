...
/* USER CODE BEGIN 1 */
/*
 * FLASHING mode runs USB MSC as host-facing transport and must respond
 * promptly to BOT/SCSI requests. Keep class thread above low-priority
 * application workers to avoid transport starvation under load.
 */
#define UX_THREAD_STACK_SIZE                 (3 * 1024)
#define UX_THREAD_PRIORITY_CLASS             5
/*
 * MSC data phase stability:
 * avoid large single-shot IN transfers that can trigger BULK-IN babble
 * on this target/stack combination during Windows enumeration probes.
 * Keep transfer chunks to one logical sector.
 */
