...
#endif
#ifndef AT25_CMD_SE64K
#define AT25_CMD_SE64K   0xD8    /* 64 KiB Sector Erase */
#endif

#define AT25_PAGE_SIZE   256u
#define AT25_ERASE64K_SIZE  (64u * 1024u)
...
static volatile AT25_Debug g_at25_dbg = {0};
static volatile AT25_Debug g_at25_dbg_err = {0};

#define AT25_DBG_OP_BOOTINIT  1u
#define AT25_DBG_OP_READ      2u
...
static void at25_dbg_note(OSPI_HandleTypeDef *h,
                          ...
                          uint32_t addr,
                          uint32_t nbytes,
                          HAL_StatusTypeDef cmd_status,
                          HAL_StatusTypeDef io_status)
{
    g_at25_dbg.last_op = op;
    g_at25_dbg.cmd_status = (uint8_t)cmd_status;
    g_at25_dbg.io_status = (uint8_t)io_status;
    g_at25_dbg.reserved0 = 0u;
    g_at25_dbg.addr = addr;
    g_at25_dbg.nbytes = nbytes;
    g_at25_dbg.hal_error = (h != NULL) ? (uint32_t)h->ErrorCode : 0u;
    g_at25_dbg.seq++;
    if (g_at25_dbg.seq == 0u) {
        g_at25_dbg.seq = 1u;
    }

    if ((cmd_status != HAL_OK) || (io_status != HAL_OK)) {
        /* Sticky first error: preserve the earliest failing transaction. */
        if (g_at25_dbg_err.seq == 0u) {
            g_at25_dbg_err.last_op = op;
            g_at25_dbg_err.cmd_status = (uint8_t)cmd_status;
            g_at25_dbg_err.io_status = (uint8_t)io_status;
            g_at25_dbg_err.reserved0 = 0u;
            g_at25_dbg_err.addr = addr;
            g_at25_dbg_err.nbytes = nbytes;
            g_at25_dbg_err.hal_error = (h != NULL) ? (uint32_t)h->ErrorCode : 0u;
            g_at25_dbg_err.seq = 1u;
        }
    }
}

/* Early init may run with interrupts masked, so HAL_Delay can hang. */
static void at25_delay_us(uint32_t us)
{
    ...
    uint32_t target = cycles_per_us * us;
    if (target == 0u) {
        target = cycles_per_us;
    }
    while ((uint32_t)(DWT->CYCCNT - start) < target) {
        __NOP();
    }
#else
    volatile uint32_t loops = cycles_per_us * us;
    while (loops-- > 0u) {
        __NOP();
    }
#endif
...
static void at25_delay_ms(uint32_t ms)
{
    while (ms-- > 0u) {
        at25_delay_us(1000u);
    }
...
bool AT25_BootInit(OSPI_HandleTypeDef *hospi, const AT25_InitCfg *cfg, uint32_t *jedec_id)
{
    ...
    g_at25_dbg_err.seq = 0u;
    at25_dbg_note(hospi, AT25_DBG_OP_BOOTINIT, 0u, 0u, HAL_OK, HAL_OK);
    return true;
}
                                   ...
                                   const uint8_t *src, uint32_t n)
{
    ...
    while (n) {
        ...
        if (st != HAL_OK) {
            at25_dbg_note(h, AT25_DBG_OP_PP, addr, chunk, st, st);
            return st;
        }
        ...
        if (st != HAL_OK) {
            at25_dbg_note(h, AT25_DBG_OP_PP, addr, chunk, st_cmd, st);
            return st;
        }

        at25_dbg_note(h, AT25_DBG_OP_PP, addr, chunk, st_cmd, HAL_OK);
...
HAL_StatusTypeDef AT25_Erase4K(OSPI_HandleTypeDef *h, uint32_t addr)
{
    ...
    HAL_StatusTypeDef st = write_enable(h, il, dl, 50);
    if (st != HAL_OK) {
        at25_dbg_note(h, AT25_DBG_OP_ERASE4K, addr, 4096u, st, st);
        return st;
    }
    ...
    st = wait_wip0(h, il, dl, 3000);
    at25_dbg_note(h, AT25_DBG_OP_ERASE4K, addr, 4096u, st_cmd, st);
    return st;
}

/* 64 KiB sector erase (addr should be 64K-aligned) */
HAL_StatusTypeDef AT25_Erase64K(OSPI_HandleTypeDef *h, uint32_t addr)
{
    uint32_t il = s_qpi_mode ? HAL_OSPI_INSTRUCTION_4_LINES : HAL_OSPI_INSTRUCTION_1_LINE;
    uint32_t al = s_qpi_mode ? HAL_OSPI_ADDRESS_4_LINES     : HAL_OSPI_ADDRESS_1_LINE;
    uint32_t dl = s_qpi_mode ? HAL_OSPI_DATA_4_LINES        : HAL_OSPI_DATA_1_LINE;
    ...
    HAL_StatusTypeDef st = write_enable(h, il, dl, 50);
    if (st != HAL_OK) {
        at25_dbg_note(h, AT25_DBG_OP_ERASE64K, addr, AT25_ERASE64K_SIZE, st, st);
        return st;
    }
    ...
    c.OperationType      = HAL_OSPI_OPTYPE_COMMON_CFG;
    c.FlashId            = HAL_OSPI_FLASH_ID_1;
    c.Instruction        = AT25_CMD_SE64K;              // 0xD8
    c.InstructionSize    = HAL_OSPI_INSTRUCTION_8_BITS;
    c.InstructionMode    = il;
    c.SIOOMode           = HAL_OSPI_SIOO_INST_EVERY_CMD;
    c.AddressMode        = al;
    ...
    // 64K erase is slower than 4K; give a longer timeout.
    st = wait_wip0(h, il, dl, 12000);
    at25_dbg_note(h, AT25_DBG_OP_ERASE64K, addr, AT25_ERASE64K_SIZE, st_cmd, st);
    return st;
}
...
void AT25_GetDebug(AT25_Debug *out)
{
    ...
    const volatile AT25_Debug *src = (g_at25_dbg_err.seq != 0u) ? &g_at25_dbg_err : &g_at25_dbg;

    for (uint32_t attempt = 0u; attempt < 2u; attempt++) {
        uint32_t seq0 = src->seq;
        if (seq0 == 0u) {
            return;
        }

        out->last_op = src->last_op;
        out->cmd_status = src->cmd_status;
        out->io_status = src->io_status;
        out->reserved0 = 0u;
        out->addr = src->addr;
        out->nbytes = src->nbytes;
        out->hal_error = src->hal_error;
