...
HAL_StatusTypeDef LCD_FlushAll(LS013B7DH05 *MemDisp, const uint8_t *buf)
{
    ...
    /* FIX: previously this used segmented blocking sends (hundreds of HAL_SPI_Transmit calls).
     * Now we build one contiguous stream into txBuf and transmit chunked. */
    uint16_t len = 0u;
