...
if g_ps_object_candidate_probe.api_version != 2
  printf "Probe mismatch: candidate API 2 required. Do not interpret other fields.\n"
else
  ...
  printf "requested/resolved-or-failed scene / validated count = %u / %u / %u\n", g_ps_object_candidate_probe.requested_scene, g_ps_object_candidate_probe.scene_id, g_ps_object_candidate_probe.scene_count
  ...
  printf "Mode 3 is private destination admission; counts describe that selected scene, not the embedded dual fixture. Existing enable helpers remain modes 1/2.\n"
