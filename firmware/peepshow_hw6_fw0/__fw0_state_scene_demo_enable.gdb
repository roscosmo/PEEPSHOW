...
printf "1. On the opening busy center STATE, watch the right-edge marker move through its three vertical positions every 250 ms. Press R: the sparse two-sprite screen appears and that right-edge marker now holds each position for two ticks while the left cursor keeps blinking every tick.\n"
...
printf "Then halt and source __fw0_state_scene_demo_prints.gdb. Expected: action errors=0, waiting-animation commits at least 2 with at least one rebase, scene replace count at least 2, replace failures=0.\n"
