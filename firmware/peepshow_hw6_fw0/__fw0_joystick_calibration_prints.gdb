...
printf "neutral phase/returns/target/stable/peak = %u / %u / %u / %u / %d\n", $sm->joystick_calibration_neutral_phase, $sm->joystick_calibration_neutral_return_count, $sm->joystick_calibration_neutral_return_target, $sm->joystick_calibration_neutral_stable_count, $sm->joystick_calibration_neutral_peak_distance
printf "neutral anchor X/Y = %d / %d\n", $sm->joystick_calibration_neutral_anchor_x, $sm->joystick_calibration_neutral_anchor_y
printf "neutral returns 0..4 X = %d / %d / %d / %d / %d\n", $sm->joystick_calibration_neutral_return_x[0], $sm->joystick_calibration_neutral_return_x[1], $sm->joystick_calibration_neutral_return_x[2], $sm->joystick_calibration_neutral_return_x[3], $sm->joystick_calibration_neutral_return_x[4]
printf "neutral returns 0..4 Y = %d / %d / %d / %d / %d\n", $sm->joystick_calibration_neutral_return_y[0], $sm->joystick_calibration_neutral_return_y[1], $sm->joystick_calibration_neutral_return_y[2], $sm->joystick_calibration_neutral_return_y[3], $sm->joystick_calibration_neutral_return_y[4]
printf "recovery attempt/success/fail/status owner/driver = %u / %u / %u / 0x%x / %u / %u\n", $sm->joystick_recovery_attempt_count, $sm->joystick_recovery_success_count, $sm->joystick_recovery_failure_count, $sm->joystick_recovery_last_status, $sm->current_state[5], $sm->joystick_driver_state
...
printf "persistent resolved/available/load/apply/save pending/status/generation = %u / %u / 0x%x / 0x%x / %u / 0x%x / %u\n", $sm->joystick_calibration_persistent_boot_resolved, $sm->joystick_calibration_persistent_load_available, $sm->joystick_calibration_persistent_load_status, $sm->joystick_calibration_persistent_apply_status, $sm->joystick_calibration_persistent_save_pending, $sm->joystick_calibration_persistent_save_status, $sm->joystick_calibration_persistent_save_generation
...
printf "neutral phases: IDLE=0 BASELINE=1 WAIT_MOTION=2 WAIT_RETURN=3 STABLE_RETURN=4\n"
