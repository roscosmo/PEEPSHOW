...
printf "display lpbam prep/prearm status/abort/status = 0x%x / 0x%x / %u / 0x%x\n", $ow->display_lpbam_prepare_status, $ow->display_lpbam_prearm_status, $ow->display_lpbam_abort_count, $ow->display_lpbam_abort_status
...
printf "display lpbam prearmed/active/cursor row/count col/count = %u / %u / %u / %u / %u / %u\n", $ow->display_lpbam_prearmed, $ow->display_lpbam_active, $ow->display_lpbam_cursor_start_row, $ow->display_lpbam_cursor_row_count, $ow->display_lpbam_cursor_start_column, $ow->display_lpbam_cursor_column_count
printf "display lpbam commit count/tick/status = %u / %u / 0x%x\n", $ow->display_lpbam_commit_count, $ow->display_lpbam_commit_tick, $ow->display_lpbam_commit_status
printf "display lpbam payload frames/chunks/bytes = %u / %u / %u\n", $ow->display_lpbam_payload_frame_count, $ow->display_lpbam_payload_chunk_count, $ow->display_lpbam_payload_bytes
...
printf "input raw driver send/drop/process/recover/status = %u / %u / %u / %u / 0x%x\n", $btn->raw_edge_send_count, $btn->raw_edge_drop_count, $btn->raw_edge_process_count, $btn->raw_edge_recovery_count, $btn->raw_edge_last_status
printf "input raw queue enq/deq/drop/high/last button/active/tick = %u / %u / %u / %u / %u / %u / %u\n", $rt->input_raw_enqueue_count, $rt->input_raw_dequeue_count, $rt->input_raw_drop_count, $rt->input_raw_queue_high_water, $rt->input_raw_last_button_id, $rt->input_raw_last_active, $rt->input_raw_last_timestamp
printf "stop2 final input check/veto/status enq/deq/queue GPIOA/B = %u / %u / 0x%x / %u / %u / 0x%x / 0x%x / 0x%x\n", $rt->stop2_final_input_check_count, $rt->stop2_final_input_veto_count, $rt->stop2_final_input_last_status, $rt->stop2_final_input_enqueue_count, $rt->stop2_final_input_dequeue_count, $rt->stop2_final_input_queue_mask, $rt->stop2_final_input_gpioa_idr, $rt->stop2_final_input_gpiob_idr
