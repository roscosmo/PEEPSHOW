...
printf "expected omnipolar switch: APIs=64/72/8; neutral remains in STOP2, cardinal movement asserts JOY_INT, IRQ enq=deq with no drops, wake source includes 0x4, primary=3, reset request=consume with pending=0, wake capture available=publish, and one logical activation is delivered per movement wake\n"
